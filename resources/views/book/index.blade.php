            ...
            <div class="col-12">
                    ...
                    <form method="get" action="{{ route('book.index') }}">
                        ...
                        <fieldset>
                            ...
                            <input type="search" name="query" value="{{ request('action') == 'search' ? request('query') : '' }}" placeholder="fulltext search">
                            ...
                            <tr>
                                <th>
                                    <input type="text" name="filter[title]" placeholder="filter by title"
                                           value="{{ request('action') == 'filter' ? request('filter.title') : '' }}">
                                </th>
                                <th>
                                    <input type="text" name="filter[description]" placeholder="filter by description"
                                           value="{{ request('action') == 'filter' ? request('filter.description') : '' }}">
                                </th>
                                <th>
                                    <input type="text" name="filter[author]" placeholder="filter by author"
                                           value="{{ request('action') == 'filter' ? request('filter.author') : '' }}">
                                </th>
                                <th>
                                    <select name="filter[category]">
                                        ...
                                        @foreach($categories as $category)
                                            <option
                                                {{ (request('action') == 'filter' && request('filter.category') == $category->id) ? 'selected' : '' }}
                                                value="{{ $category->id }}">
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                ...
                                <th>
                                    <input type="date" name="filter[date]" placeholder="filter by date"
                                           value="{{ request('action') == 'filter' ? request('filter.date') : '' }}">
                                </th>
                                <th>
